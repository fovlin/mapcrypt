...
func main() {
	...
	srcData, err := os.ReadFile(codeTableFile)
	if err != nil {
	...
	err_2 := json.Unmarshal(srcData, &codeTable)
	if err_2 != nil {
...
func encrypt(codeTable map[string]string) {
	...
	data, err := json.Marshal(codeList)
	if err != nil {
		fmt.Println("[ERROR]", err)
		return
	}
...
func decrypt(codeTable map[string]string) {
	...
	var codeList []string
	err_2 := json.Unmarshal(code, &codeList)
	if err_2 != nil {
		fmt.Println("[ERROR]", err_2)
		return
	}
